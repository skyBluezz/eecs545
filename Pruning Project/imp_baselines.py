...
def cal_importance_dataDriven_conv(net, l_id, num_stop=100):
    print('Working 1')
    optimizer = optim.SGD(net.parameters(), lr=0, weight_decay=0)
    optimizer = optim.SGD(net.parameters(), lr=0, weight_decay=0)
    num = 0
       ...
       # print(l_id_output.shape)
        ...
        for a in range(0,l_id_output.shape[1]):
               # print(l_id_output[:,a,:,:].shape)
                sum_ = 0
                sum_ = (0,np.count_nonzero(l_id_output.detach().cpu().numpy()[:,a,:,:] == 0))
                aPoZ[a] += sum_[1]*0.001
...
def cal_importance_dataDriven_linear(net, l_id, num_stop=100):
    print('Working 1')
    optimizer = optim.SGD(net.parameters(), lr=0, weight_decay=0)
    optimizer = optim.SGD(net.parameters(), lr=0, weight_decay=0)
    num = 0
    size = net.classifier[l_id-1].weight.shape[0]
    aPoZ = torch.zeros(size)
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        l_id_output = net.classifier[0:l_id+1](net.features(inputs).squeeze())  #output of the relu layer 
       # print(l_id_output.shape)
        for a in range(0,l_id_output.shape[1]):
                sum_ = 0
                sum_ = (0,np.count_nonzero(l_id_output.detach().cpu().numpy()[:,a] == 0))
